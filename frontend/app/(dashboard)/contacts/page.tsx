...
function getThisMonthCount(list: Contact[]) {
  const now = new Date();
  const m = now.getMonth() + 1;
  const y = now.getFullYear();

  return list.filter((c) => {
    const [yy, mm] = normalizeDate(c.createdAt).split("-").map(Number);
    return yy === y && mm === m;
  }).length;
}

function getThisYearCount(list: Contact[]) {
  const y = new Date().getFullYear();
  return list.filter(
    (c) => Number(normalizeDate(c.createdAt).slice(0, 4)) === y
  ).length;
}

function getTodayCount(list: Contact[]) {
  ...
  return list.filter((c) => normalizeDate(c.createdAt) === todayStr).length;
}

function getTopSource(list: any[]) {
  if (!list.length) return "-";
  const map: Record<string, number> = {};
  list.forEach((c) => {
    const src = (c.source ?? c.source_page ?? "Unknown").trim();
    map[src] = (map[src] || 0) + 1;
  });
...
export default function ContactsPage() {
  ...
  const monthCount = useMemo(() => getThisMonthCount(contacts), [contacts]);
  const yearCount = useMemo(() => getThisYearCount(contacts), [contacts]);
  const todayCount = useMemo(() => getTodayCount(contacts), [contacts]);
  const topSource = useMemo(() => getTopSource(contacts as any), [contacts]);

  const handleView = (row: Contact) => {
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Manage and track your contacts efficiently
        </p>
      </div>
      ...
      {loading ? (
        ...
        <ContactsTable data={contacts} onView={handleView} />
      )}

      {/* ✅ Modal with blur */}
      {openView && selectedContact && (
          ...
          <div className="relative z-10 w-full max-w-xl bg-white rounded-2xl shadow-xl border overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                <Detail
                  label="ID"
                  value={String((selectedContact as any).id ?? "-")}
                />
                <Detail
                  label="Created Date"
                  value={normalizeDate(selectedContact.createdAt)}
                />
                ...
                <Detail
                  ...
                  value={
                    ...
                    (selectedContact as any).source_page ||
                    "-"
                  }
                />
                <Detail
